<template>
    <ion-page>
      ...
      <ion-content :fullscreen="true">
        ...
        <ion-grid>
          ...
          <ion-row >
            <ion-col >Last 7 days Medecine Intake</ion-col>
          </ion-row>
        </ion-grid>
        <ion-grid>
          <ion-list>
              <ion-item v-for="med in filteredMedecine">
                <ion-label>{{ med.date+' - '+med.medecine }}</ion-label>
              </ion-item>
            </ion-list>
        </ion-grid>
  ...
  const { getLast7DaysById  } =  useAppWriteMedication();
  ...
  const medecineTaken = ref();
  const filteredMedecine= ref();

  
  const initialize = async() => {
      showLoading();
      medecineTaken.value = (await getLast7DaysById(userid.value??'')).newMedecine;
      setTimeout(()=>{
        filteredMedecine.value =medecineTaken.value;
        },3000)
      
      console.log(medecineTaken.value);
